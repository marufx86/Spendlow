...
const LendingList: React.FC<LendingListProps> = ({ className }) => {
  const { filteredLendings, deleteLending, isLoading } = useBudget();
  ...
  const showAll = () => {
    setDisplayLimit(filteredLendings.length);
  };
    ...
    <Transition appear="fade" className={cn(
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 px-4 rounded-lg bg-accent/50">
          <p className="text-muted-foreground">No lending records found</p>
        ...
        <ScrollArea className="rounded-md border h-[calc(100vh-28rem)] min-h-[300px]">
          <Table>
            ...
            <TableBody>
              {filteredLendings.slice(0, displayLimit).map((lending) => (
                <TableRow 
                  key={lending.id}
                  className={cn(
                    "transition-all duration-300",
                    isDeleting === lending.id && "opacity-0 scale-95"
                  )}
                >
                  <TableCell className="font-mono text-xs text-muted-foreground">
                    {new Date(lending.date).toLocaleDateString()}
                  </TableCell>
                  <TableCell className="font-medium">{lending.person}</TableCell>
                  ...
                  <TableCell>
                    <span className={cn(
                      ...
                      {lending.type === 'lent' ? 'Lent Out' : 'Borrowed'}
                    </span>
                  </TableCell>
                  <TableCell className="text-right font-medium">
                    {formatter.format(lending.amount)}
                  </TableCell>
